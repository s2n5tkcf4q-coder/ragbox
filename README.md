...
基于大语言模型（LLM）的 RAG（Retrieval‑Augmented Generation）智能问答系统，支持 **API** 与 **本地 Ollama** 两种模式，可用于个人及企业文档、规章制度、报告等领域的知识管理与检索式问答。 系统本地部署使用ollama所有数据及运算都在本地处理运行，**可放心处理隐私和绝密文件** ，不存在泄密的可能。**所有参数均可自行调整，自行控制答案质量**。
...
- 相似度阈值：判断“多像才算相关”的分数线，返回内容不太相关就调高，返回内容漏掉相关内容就调低。
- TOP-N：从向量数据库中找出与用户问题最相似的文本块数量，5个一般够用，调低可能会漏掉相关内容，调高会增大上下文长度token数。
- 向量相似度权重：在混合检索中，给语义相似度分数占最终分数的比例。调低则匹配精确，调高则更强调语义。
- 温度：回答准确度，越低越准确但刻板；越高创意越强但容易跑题。
- TOP-P：核心采样参数；与选择LLM模型有关，希望稳定输出可调低。
- 最大Token数：最多允许输出的token数量。它限制的是输出长度。可根据模型参数大小自行调整。
- 存在惩罚：降低模型再次讨论已经出现过的“话题”的可能性。
- 频率惩罚：通过对出现次数越多的词施加越重的惩罚，来抑制模型反复使用同一个词或短语。
- 文件处理时文本块最大字符数为500，相邻块重叠字符数为50。用户如需调整可在config.py文件中修改。
```python
    # 文本分块参数
